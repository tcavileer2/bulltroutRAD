...
my $e = 5;		# set length of remaining enzyme cutsite sequence (e.g. 6 for SbfI; 5 for PstI, NsiI)
...
while($name1 = <IN1>) {		# start reading through the paired fastq input files
	...
	if(substr($forward, $c, $e) eq "TGCAG" || substr($forward, $c, $e) eq "TGCAT") {			# check for cutsite in the correct place in forward read
		if(substr($reverse, $c, $e) eq "TGCAG" || substr($reverse, $c, $e) eq "TGCAT") {		# check for cutsite in the correct place in reverse read
	...
	elsif(substr($reverse, $c, $e) eq "TGCAG" || substr($reverse, $c, $e) eq "TGCAT") {		# cutsite not in forward read but is in reverse read
